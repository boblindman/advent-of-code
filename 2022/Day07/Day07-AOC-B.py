...
minimumDiskSpaceRequired = 30000000
def upDir(theDir):
    # Find the last /
...
if __name__ == "__main__":
    ...
    availableSpace = totalDiskSpace - totalSizeOfDirs["/"]
    print("Available disk space to start: {}".format(availableSpace))

    setOfPossibleAnswers = []    
    for key in totalSizeOfDirs:
        if (availableSpace + totalSizeOfDirs[key]) > minimumDiskSpaceRequired:
            setOfPossibleAnswers.append(totalSizeOfDirs[key])
    print(setOfPossibleAnswers)
    myAnswer = min(setOfPossibleAnswers)
